...
/**
 ...
 INTERRUPT_HANDLER(UART1_TX_IRQHandler, 17)
 {
    ...
    */
	...
	{
		uart.txEmpty = TRUE;
        //UART1->CR2 &= ~0x80;  //发送中断不使能
        UART1_ITConfig(UART1_IT_TXE, DISABLE);
	}
	else
	{
        UART1_SendData8(uart.txBuf[uart.txTail]);
		uart.txTail = (uart.txTail + 1) & (UART_TXBUFFERLEN - 1);
        uart.txcount = TXTIMEOUT;
	}
    #if defined (RS485_SEND)
        rs485_send_timeout = 2000;
    #endif
 }
...
/**
  ...
  */
    ...
    if (uart.rxCount < UART_RXBUFFERLEN)
    {
    	uart.rxBuf[uart.rxCount++] = UART1_ReceiveData8();
    	uart.rxTimeOut = RXTIMEOUT;
    }
}
